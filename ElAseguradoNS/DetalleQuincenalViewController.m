//
//  DetalleQuincenalViewController.m
//  ElAseguradoNS
//
...
#import "DetalleQuincenalViewController.h"
#import "AseguradorIAPHelper.h"
#import <StoreKit/StoreKit.h>

@interface DetalleQuincenalViewController ()

@end

@implementation DetalleQuincenalViewController
{
    ...
    NSDictionary * cuarto;
    NSNumber *rowClicked;
    NSString *linkIzquierdo, *linkDerecho, *prefaceString;
...
- (void)viewDidLoad
{
    ...
    //NSLog(@"Mes: %@ Quincena: %i", rowClicked, cuartoNum);
    cuarto = meses[[rowClicked intValue]][cuartoNum];
    linkIzquierdo = [cuarto objectForKey:@"anuncioLinkIzquierdo"];
    linkDerecho   = [cuarto objectForKey:@"anuncioLinkDerecho"];
    
    if([deviceType isEqualToString:@"iPhone"] || [deviceType isEqualToString:@"iPod touch"])
        prefaceString = @"iPhone";
    else
        prefaceString = @"iPad";
    
    preface.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@%@.jpg", prefaceString, [cuarto objectForKey:@"preface"]]];
    
    self.navigationItem.title = [cuarto objectForKey:@"PDF"];
    //IAPs
    ...
    [[AseguradorIAPHelper sharedInstance] requestProductsWithCompletionHandler:^(BOOL success, NSArray *products) {
        if (success) {
            ...
            /* //Checar el orden de los IAP, resulta que todos están igual que en iTunes Connect
             for (SKProduct * skProduct in _products) {
             NSLog(@"%@", skProduct.localizedTitle);
             }*/
...
- (void)productPurchased:(NSNotification *)notification {
    ...
    [_products enumerateObjectsUsingBlock:^(SKProduct * product, NSUInteger idx, BOOL *stop) {
        if ([product.productIdentifier isEqualToString:productIdentifier]) {
            UIAlertView *message = [[UIAlertView alloc] initWithTitle:@"Gracias por la compra"
...
- (IBAction)buyButtonQ1 {
  ...
  /*
     ...
     product = _products[x];
     NSLog(@"%i - %@", x , product.localizedTitle);
     }*/
...
-(IBAction)segueEntrarQ1{
    ...
    [self performSegueWithIdentifier: @"segueDetalleNoticias" sender: self];
}
...
- (IBAction)anuncioIzq {
    [[UIApplication sharedApplication] openURL:[NSURL URLWithString: linkIzquierdo]];
}

-(IBAction)anuncioDer{
    [[UIApplication sharedApplication] openURL:[NSURL URLWithString: linkDerecho]];
}
